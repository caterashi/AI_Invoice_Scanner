...
Ključevi MORAJU biti TAČNO ovi:
{
  ...
  "DATUMF": "Datum izdavanja fakture/računa (format DD.MM.GGGG)",
  "DATUMPF": "Datum prijema fakture samo ako eksplicitno postoji, inače prazan string",
...
Najvažnije pravilo za DATUMF:
- DATUMF mora biti datum IZDAVANJA fakture/računa.
- Prioritetne oznake su: "Datum izdavanja fakture", "Datum izdavanja računa", "Datum računa", "Dat. izdavanja".
- Ako postoji spojeno polje tipa "Dat. izdavanja/valute: 31.01.2026 / 02.03.2026", uzmi PRVI datum kao DATUMF.
- Nikada ne koristi kao DATUMF: datum isporuke, obračunski period, valuta, rok plaćanja, datum očitanja, datum plaćanja, KIF/KF/UF/MJF datum ili proknjiženo datum.

Pravila:
- Čitaj samo ono što je vidljivo u dokumentu.
- Ne koristi naziv fajla kao izvor podataka.
- NAZIVPP i SJEDISTEPP moraju biti PODACI O KUPCU / PRIMAOcu RAČUNA, ne o dobavljaču.
- IDPDVPP i JIBPUPP uzimaj od DOBAVLJAČA / izdavaoca računa.
- DATUMPF popuni samo ako dokument eksplicitno sadrži datum prijema, zaprimanja, evidentiranja ili ulazni datum.
- Ako nisi siguran da li neki datum predstavlja izdavanje fakture, ostavi DATUMF prazan.
...
def _find_invoice_number(text: str) -> str:
    ...
    patterns = [
        ...
        r"(?:RAČUN|RACUN|FAKTURA)[^\d]{0,35}([A-Z]?[-/]?\d+[/-]\d{2,4})",
        r"(?:RAČUN|RACUN|FAKTURA)[^\n]{0,20}\n?([0-9][0-9\-/]{5,30})",
    ]
    for pat in patterns:
...
def _has_new_invoice_signal(text: str) -> bool:
    ...
    if _find_invoice_number(t):
        return True
    signals = [
        r"(?:ra[čc]un|faktura)\s*[:\-]",
        r"(?:datum\s+ra[čc]una|datum\s+izdavanja\s+fakture|datum\s+izdavanja\s+ra[čc]una)",
    ]
...
def _extract_header_supplier_ids_from_text(text: str) -> dict[str, str]:
    t = text or ""
    top = t[:2200]
    cut_markers = [r"Kupac", r"Buyer", r"Primatelj", r"Primaoc", r"Naručilac", r"Narucilac", r"RAČUN", r"RACUN", r"FAKTURA"]
    cut_pos = len(top)
    ...
    supplier_id = ""
    supplier_pdv = ""
    id_match = re.search(r"(?:ID\s*(?:BROJ|br\.?|broj)\s*:?)\s*(\d{12,13})", head, flags=re.IGNORECASE)
    pdv_match = re.search(r"(?:PDV\s*(?:BROJ|br\.?|broj)\s*:?)\s*(\d{11,13})", head, flags=re.IGNORECASE)
    if id_match:
        supplier_id = InvoiceData.normalize_idpdvpp(id_match.group(1))
    if pdv_match:
        supplier_pdv = InvoiceData.normalize_jibpupp(pdv_match.group(1))
    if supplier_id and not supplier_pdv:
        d = re.sub(r"\D", "", supplier_id)
        if re.fullmatch(r"4\d{12}", d):
            supplier_pdv = d[1:]
    if supplier_pdv and not supplier_id:
        d = re.sub(r"\D", "", supplier_pdv)
        if re.fullmatch(r"\d{12}", d):
            supplier_id = "4" + d
    return {"IDPDVPP": supplier_id, "JIBPUPP": supplier_pdv}


def _extract_header_buyer_from_text(text: str) -> dict[str, str]:
    t = str(text or "")
    if not t.strip():
        return {"NAZIVPP": "", "SJEDISTEPP": ""}
    top = t[:3500]
    m = re.search(r"(?:Kupac|Buyer|Primatelj|Primaoc|Naručilac|Narucilac)\s*:?\s*(.+?)(?=(?:Račun|RAČUN|Racun|RACUN|Faktura|FAKTURA|Datum\s+izdavanja|Datum\s+ra[čc]una|Mjesto\s+izdavanja|$))", top, flags=re.IGNORECASE | re.DOTALL)
    if not m:
        return {"NAZIVPP": "", "SJEDISTEPP": ""}
    block = m.group(1)
    lines = [re.sub(r"\s+", " ", x).strip(" ,|-") for x in re.split(r"\n+", block) if re.sub(r"\s+", " ", x).strip()]
    cleaned = []
    for line in lines:
        if re.search(r"^(IDB|ID\s+BROJ|PDV\s+BROJ)\s*:?", line, flags=re.IGNORECASE):
            break
        cleaned.append(line)
    if not cleaned:
        one_line = re.sub(r"\s+", " ", block).strip(" ,|-")
        one_line = re.split(r"IDB\s*:|ID\s+BROJ\s*:|PDV\s+BROJ\s*:", one_line, maxsplit=1, flags=re.IGNORECASE)[0]
        cleaned = [x.strip() for x in re.split(r"\s{2,}|,\s*", one_line) if x.strip()]
    name = cleaned[0] if cleaned else ""
    addr = " ".join(cleaned[1:]).strip() if len(cleaned) > 1 else ""
    return {"NAZIVPP": name, "SJEDISTEPP": addr}


def _extract_issue_date_from_text(text: str) -> str:
    t = text or ""
    patterns = [
        r"Datum\s+izdavanja\s+fakture\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
        r"Datum\s+izdavanja\s+ra[čc]una\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
        r"Dat\.\s*izdavanja\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
        r"Datuma\s+ra[čc]una\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
        r"Datum\s+ra[čc]una\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
        r"Dat\.\s*izdavanja\s*/\s*valute\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})\s*/\s*\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4}",
        r"Dat\.\s*izdavanja/valute\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})\s*/\s*\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4}",
    ]
    for pat in patterns:
        m = re.search(pat, t, flags=re.IGNORECASE)
        if m:
            return InvoiceData.normalize_date(m.group(1))
    return ""


def _extract_receipt_date_from_text(text: str) -> str:
    t = text or ""
    patterns = [
        r"Datum\s+prijema\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
        r"Datum\s+zaprimanja\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
        r"Ulazni\s+datum\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
        r"Evidentirano\s+dana\s*:?\s*(\d{1,2}[./\-]\s*\d{1,2}[./\-]\s*\d{4})",
    ]
    for pat in patterns:
        m = re.search(pat, t, flags=re.IGNORECASE)
        if m:
...
def _apply_text_priority(inv: InvoiceData, source_text: str) -> InvoiceData:
    buyer = _extract_header_buyer_from_text(source_text)
    supplier_ids = _extract_header_supplier_ids_from_text(source_text)
    issue_date = _extract_issue_date_from_text(source_text)
    receipt_date = _extract_receipt_date_from_text(source_text)
    if buyer.get("NAZIVPP"):
        inv.NAZIVPP = buyer["NAZIVPP"]
    if buyer.get("SJEDISTEPP"):
        inv.SJEDISTEPP = buyer["SJEDISTEPP"]
    if supplier_ids.get("IDPDVPP"):
        inv.IDPDVPP = supplier_ids["IDPDVPP"]
    if supplier_ids.get("JIBPUPP"):
        inv.JIBPUPP = supplier_ids["JIBPUPP"]
    elif inv.IDPDVPP:
        d = re.sub(r"\D", "", inv.IDPDVPP or "")
        if re.fullmatch(r"4\d{12}", d):
            inv.JIBPUPP = d[1:]
    if issue_date:
        inv.DATUMF = issue_date
    if receipt_date:
        inv.DATUMPF = receipt_date
    else:
        inv.DATUMPF = ""
    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Fajl: {filename}\nSegment: {label or 'tekst'}\n\nVrati samo račune vidljive u ovom segmentu. Posebno pazi da DATUMF bude datum izdavanja računa/fakture, a ne datum isporuke ili valute.\n\nTEKST DOKUMENTA:\n{text}"},
            ],
        )
...
def _extract_via_vision_segment(b64_image: str, filename: str, label: str = "") -> list[InvoiceData]:
    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": _SYSTEM_PROMPT},
                {"role": "user", "content": [{"type": "text", "text": f"Fajl: {filename}\nSegment: {label or 'slika'}\n\nVrati samo račune vidljive u ovom segmentu. DATUMF mora biti datum izdavanja računa/fakture, nikako datum isporuke, valute ili plaćanja."}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64_image}"}}]},
            ],
        )
        items = _parse_response(resp.choices[0].message.content.strip(), f"{filename}{' ' + label if label else ''}")
        for inv in items:
...
def _supplier_key(inv: InvoiceData) -> str:
    ...
    if inv.BROJFAKT:
        parts.append((inv.BROJFAKT or "").strip().lower())
...
def _merge_invoice_group(group: list[InvoiceData]) -> InvoiceData:
    if len(group) == 1:
        ...
        return inv
    merged = InvoiceData()
    for field in ["BROJFAKT", "DATUMF", "DATUMPF", "NAZIVPP", "SJEDISTEPP", "IDPDVPP", "JIBPUPP"]:
    ...
    for field in ["IZNBEZPDV", "IZNPDV", "IZNSAPDV"]:
    ...
    if merged._source_text:
        _apply_text_priority(merged, merged._source_text)
...
def _merge_duplicate_invoices(items: list[InvoiceData]) -> list[InvoiceData]:
    ...
    for inv in clean:
        ...
        if not placed:
            groups.append([inv])
    return [_merge_invoice_group(g) for g in groups]
...
def _choose_best_per_invoice(items: list[InvoiceData]) -> list[InvoiceData]:
    ...
    for inv in items:
        ...
        if broj:
            ...
            no_num.append(inv)
    final = []
    for _, group in by_num.items():
        best = sorted(group, key=_invoice_strength, reverse=True)[0]
...
def _finalize_results(items: list[InvoiceData]) -> list[InvoiceData]:
    ...
    return _normalize_numeric_id_strings(best) or [_error_invoice("", "Nema rezultata")]
